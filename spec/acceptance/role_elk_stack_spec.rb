...
describe 'role::elk_stack' do
  ...
  context 'configuration files' do
    describe file('/etc/filebeat/inputs.d/syslogs.yml') do
      its(:content) { is_expected.to match(/type: filestream/) }
    end

    describe file('/etc/logstash/conf.d/main.conf') do
      its(:content) { is_expected.to match %r{http://localhost:9200} }
    end

    describe file('/etc/kibana/kibana.yml') do
      its(:content) { is_expected.to match(/elasticsearch.hosts/) }
    end

    describe file('/etc/elasticsearch/elasticsearch.yml') do
      its(:content) { is_expected.to match(/discovery.type.*single-node/) }
      its(:content) { is_expected.to match(/xpack.security.enabled.*false/) }
    end
  end
  ...
  context 'ports and APIs' do
    ...
    describe command('curl -s http://localhost:9200/_cluster/health?pretty') do
      its(:stdout) { is_expected.to match(/green|yellow/) }
    end
