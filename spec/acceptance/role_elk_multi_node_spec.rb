...
service_expectation = role_expectations.dig(expected_role,
                                            :service) || 'runs the service and listener for the target role'
config_expectation = role_expectations.dig(expected_role,
                                           :config) || 'writes the endpoint configuration for the target role'
...
describe "role::elk_multi_node on #{role_label}" do
  ...
  context 'role-specific cross-node configuration' do
    it config_expectation do
      ...
      when 'edge'
        expect(command('nginx -T 2>/dev/null').stdout).to match(/10\.43\.1\.12:5601/)
      end
    end
  end
end
